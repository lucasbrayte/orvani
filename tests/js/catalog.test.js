...
test("catalog professional showcase selects and rotates featured products", () => {
  assert.equal(typeof core.featuredProducts, "function");
  assert.equal(typeof core.featuredProductWindow, "function");
  ...
  const products = [
    ...
    { id: "f", active: true, featured: true },
    { id: "g", active: true, featured: true },
  ];
  ...
  assert.deepEqual(
    [...core.featuredProductWindow(products, 0, 4)].map((product) => product.id),
    ["a", "d", "e", "f"],
  );
  assert.deepEqual(
    [...core.featuredProductWindow(products, 4, 4)].map((product) => product.id),
    ["g", "a", "d", "e"],
  );
});

test("catalog professional categories count only active products", () => {
  assert.equal(typeof core.catalogCategoryPresentation, "function");

  const result = core.catalogCategoryPresentation([
    { active: true, category: "Games" },
    { active: true, category: "Games" },
    { active: true, category: "Eletrônicos" },
    { active: false, category: "Games" },
  ]);

  assert.deepEqual(
    [...result].map(({ title, slug, count }) => ({ title, slug, count })),
    [
      { title: "Games", slug: "games", count: 2 },
      { title: "Eletrônicos", slug: "eletronicos", count: 1 },
    ],
  );
});

test("catalog professional storefront exposes featured and category hosts", () => {
  ...
  for (const id of [
    "catalog-featured",
    "catalog-featured-primary",
    "catalog-featured-secondary",
    "catalog-featured-indicators",
    "catalog-category-track",
  ]) {
    assert.match(html, new RegExp(`id="${id}"`));
  }

  assert.match(css, /Professional catalog storefront redesign/);
  assert.match(css, /\.catalog-featured-stage\s*\{/);
  assert.match(
    css,
    /@media \(min-width:\s*84rem\)[\s\S]*?repeat\(4,\s*minmax\(0,\s*1fr\)\)/,
  );
  assert.match(script, /const intervalMs = 5000;/);
  assert.match(script, /function createCatalogFeaturedRotator/);
  assert.match(script, /function renderCatalogCategoryStrip/);
});
